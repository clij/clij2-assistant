...
public class MemoryDisplay implements PlugIn, ImageListener {
    ...
    public void run(String arg) {
        if (viewer == null) {
            viewer = new ImagePlus(CLIJ2.getInstance().getGPUName(), generateDisplay());
            viewer.show();
    ...
    private ImageProcessor generateDisplay() {
        ...
        measurements[measurement_count - 1] = getMemoryConsumption(CLIJ2.getInstance());
...
//        TextRoi roi = new TextRoi(status, 0, 15, font);
  //      roi.setStrokeColor(Color.black);
    //    viewer.setRoi(roi);
        ...
        ip.setColor(Color.black);
        ip.drawString(CLIJ2.getInstance().getGPUName() + "\n" + status, 3, 17);
        return ip;
    }

    private static float getMemoryConsumption(CLIJ2 instance) {
    ...
    public static void main(String... args) throws InterruptedException {
        ...
        ClearCLBuffer buffer1 = clij2.create(1024, 1024, 100);
        ClearCLBuffer buffer2 = clij2.create(1024, 1024, 50);
        ClearCLBuffer buffer3 = clij2.create(1024, 1024, 150);
    ...
    public static String getStatus() {
        getMemoryConsumption(CLIJ2.getInstance());
        return status;
    }
}
