...
import net.haesleinhuepf.clij2.assistant.services.*;
...
import net.haesleinhuepf.clij2.assistant.interactive.handcrafted.Crop2D;
...
import static net.haesleinhuepf.clij2.assistant.utilities.AssistantUtilities.distributionName;
import static net.haesleinhuepf.clij2.assistant.utilities.AssistantUtilities.*;
...
public abstract class AbstractAssistantGUIPlugin implements ImageListener, PlugIn, AssistantGUIPlugin {
    ...
    public void setCLIJMacroPlugin(CLIJMacroPlugin plugin) {
        ...
        if (plugin != null) {
            plugin.setClij(CLIJ2.getInstance().getCLIJ());

            if (plugin instanceof AbstractCLIJ2Plugin) {
                ((AbstractCLIJ2Plugin) plugin).setCLIJ2(CLIJ2.getInstance());
            }
        }
    }
    ...
    protected void showTable() {
        ...
        clij2.pullToResultsTable(result[0], my_table);
    ...
    protected ClearCLBuffer[] createOutputBufferFromSource(ClearCLBuffer[] pushed) {
        ...
        if (pushed.length > 1) {
            ...
            for (int i = 1; i < pushed.length; i ++) {
                output[i] = clij2.create(output[0]);
            }
...
*/
        ...
        addMenuAction(info,"GPU: " + CLIJ2.getInstance().getGPUName(), (a) -> {
            IJ.log(CLIJ2.clinfo());
        });
    ...
    protected synchronized void enhanceContrast() {
        ...
        for (int c = 0; c < my_target.getNChannels(); c++) {
            ...
            if (plugin instanceof VisualizeOutlinesOnOriginal) {
                ...
                IJ.setMinAndMax(my_target,CLIJ2.getInstance().minimumOfAllPixels(result[0]) - 1, CLIJ2.getInstance().maximumOfAllPixels(result[0]));
            } else if (labelimage) {
                System.out.println("Set 0 max");
                IJ.setMinAndMax(my_target,0, CLIJ2.getInstance().maximumOfAllPixels(result[0]));
            } else if (binary) {
    ...
    protected void restoreParameters(ParameterContainer container) {
        AssistantGUIPlugin[] path = AssistantGUIPluginRegistry.getInstance().getPathToRoot(this);
        Object[][] parameters = OptimizationUtilities.getParameterArraysFromIncubatorPlugins(path);
